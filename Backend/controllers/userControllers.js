...
const polygonData = AsyncHandler(async (req, res) => {
  ...
  try {
    h = await polygon.find({ Email: email });
    if (h.length) {
...
const addressFind = AsyncHandler(async (req, res) => {
  ...
  // Write logic if lat and lon already present
  try {
    let h = await user.findOne({ Email: email });
    console.log(h);
    if (h.Lat && h.Lon) {
      res.json({ lat: h.Lat, lon: h.Lon })
    }
    else {
      res.json({
        address: h.Address,
    ...
    if (h.length) {
      ...
      for (let i = 0; i < h.length; i++) {
        ...
        try {
          ...
          if (h[i].Email === '8') {
            console.log("this is something", h1)
            console.log("Polygon_id");
            console.log(polygon_id)
          }
          try {
...
const chatUser = AsyncHandler(async (req, res) => {

});

const postUser = AsyncHandler(async (req, res) => {
  const { profile, username, timestamp, description } = req.body;
  try {
    now = await postsconsider.create({ profile: profile, username: username, timestamp: timestamp, description: description, likes: 0 });
    await userpostconsider.create({ post_id: now._id, liked_arr: [], disliked_arr: [] });
    res.json("Completed")
  }
  catch (err) {
    console.log(err);
...
const likesLogic = AsyncHandler(async (req, res) => {
  const { _id, likes } = req.body;
  console.log(_id);
  console.log(likes);
  try {
    ans = await postsconsider.find({ _id: _id });
    // console.log(ans)
    await ans[0].updateOne({ likes: likes });
    res.json("Likes completed");
  }
  catch (err) {
    res.json("Failed for some reason")
  }
})

const showLikeButtonLogic = AsyncHandler(async (req, res) => {
  const { post_id, user_id, like } = req.body;
  try {
    if (like == 1) {
      console.log("Happeneded")
      ans = await userpostconsider.findOne({ post_id: post_id });
      arr = []
      console.log(ans)
      if (ans.length == 0) {
        arr = []
        ...
        arr = ans.liked_user_id;
        arr.push(user_id);
      }
      await ans.updateOne({ liked_user_id: arr });
      ...
      if (ans.length == 0) {
        ...
        arr = ans.disliked_user_id;
        arr.push(user_id);
      }
      await ans.updateOne({ disliked_user_id: arr });
    }
    res.json("Sucess in finding array")
  }
  catch (err) {
    console.log(err);
    res.json("Failed in finding array")
  }
})

const dontShowLikeButtonLogic = AsyncHandler(async (req, res) => {
  const { post_id, user_id, like } = req.body;
  ...
  try {
    if (like == 1) {
      ans = await userpostconsider.findOne({ post_id: post_id });
      arr = ans.liked_user_id;
      arr = arr.filter((curr_ele, curr_idx, curr_arr) => {
        return curr_ele != user_id
      })
      await ans.updateOne({ liked_user_id: arr });
    }
    else {
...
const getPostLikes = AsyncHandler(async (req, res) => {
  const { _id } = req.body;
  try {
    d = await userpostconsider.findOne({ post_id: _id });
    console.log(_id)
    console.log(d);


    res.json({ liked_arr: d.liked_user_id, disliked_arr: d.disliked_user_id });
  }
  catch (err) {
    console.log(err);
    res.json(err);
